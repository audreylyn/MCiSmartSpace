...
include '../auth/dbh.inc.php';

// Check if the equipment already has an active report
$existingReport = null;
$existingReportId = null;
$existingReportRef = null;

if (!empty($equipmentId)) {
    $checkReportSql = "SELECT ei.id, ei.status, 
                      CASE WHEN ei.student_id IS NOT NULL THEN 'Student' ELSE 'Teacher' END as reporter_type,
                      ei.reported_at 
    ...
    $checkStmt = $conn->prepare($checkReportSql);
    ...
    if ($result->num_rows > 0) {
        ...
        $existingReportRef = 'EQ' . str_pad($existingReportId, 6, '0', STR_PAD_LEFT);
    }
    $checkStmt->close();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_report'])) {
    // Check again for existing reports to prevent race conditions
    if (!empty($equipmentId)) {
        $checkReportSql = "SELECT id FROM equipment_issues 
                          WHERE equipment_id = ? AND (status = 'pending' OR status = 'in_progress')
                          LIMIT 1";
        ...
        if ($result->num_rows > 0) {
            ...
            $existingReportId = $existingReport['id'];
            $existingReportRef = 'EQ' . str_pad($existingReportId, 6, '0', STR_PAD_LEFT);
            $error_message = "Another report (Reference: $existingReportRef) was recently submitted for this equipment. Please check the equipment reports page.";
            $checkStmt->close();
    ...
    if ($teacherId && !isset($error_message)) {
        ...
        try {
            if ($stmt->execute()) {
                ...
                // Store report ID in session for confirmation page
...
// Add this label at the end of the file, right before the HTML closing tag
display_page:
?>
        ...
        <div class="issue-report-container">
            ...
            <?php if ($existingReport): ?>
                <div class="alert alert-warning">
                    <strong>Note:</strong> This equipment already has an active
                    <?php echo isset($existingReport['status']) ? strtolower($existingReport['status']) : 'pending'; ?> report
                    (Reference: <?php echo $existingReportRef; ?>) submitted by a
                    <?php echo isset($existingReport['reporter_type']) ? $existingReport['reporter_type'] : 'user'; ?>
                    on <?php echo isset($existingReport['reported_at']) ? date('M d, Y', strtotime($existingReport['reported_at'])) : date('M d, Y'); ?>.
                    <p>To avoid duplication, please check the existing report status in your <a href="equipment_report_status.php">Equipment Reports</a> page.</p>
                </div>
            <?php endif; ?>
